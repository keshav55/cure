...
\section{Mutation-Level Scoring}

Scoring at the mutation level (48,306 mutations, 213 positive) rather
than peptide level achieves recall@20 of 0.547---higher than the
peptide-level ceiling of 0.505. The mutation-level model uses expression
and driver features without the noise of multiple peptide variants per
mutation. Two independent approaches (peptide-level gated GBT+RF and
mutation-level GBT with peptide retrieval) converge on the same
peptide-level recall@20 of 0.505, confirming this as the data ceiling.

\section{Feature Ablation}

Only three feature groups matter for neoantigen prediction:
...
\begin{tabular}{lr}
\toprule
\textbf{Features} & \textbf{R@20} \\
\midrule
Binding + expression & 0.420 \\
+ Stability rank & 0.473 (+0.053) \\
+ TAP + cleavage & 0.465 ($-$0.008) \\
+ Driver + clonality & 0.466 (+0.001) \\
All 19 features & 0.498 (+0.032) \\
\bottomrule
\end{tabular}
\caption{Feature ablation. Stability adds +0.053; TAP/cleavage add noise.}
\end{table}

\section{Comparison with DeepImmuno}

On the TESLA dataset, our approach achieves AUC 0.995 vs.\ DeepImmuno
CNN's 0.654. DeepImmuno uses only peptide sequence and HLA allele;
we additionally use expression, variant allele support, and stability.
Expression data (alt\_support $p = 5 \times 10^{-78}$) provides more
discriminative power than any sequence-only model.
...
\item \textbf{Simple scores at peptide level fail}: at peptide level,
no formula replicates ML ($p = 0.003$). However, at \textit{mutation}
level, alt\_support $\times$ TPM achieves 0.550 recall@20---matching
the ML model (0.540). Immunogenicity is primarily determined by
total mutant mRNA production
\item \textbf{Patient prediction fails}: LOOCV AUC 0.420; 73.7\%
of patients have $\geq 1$ positive, so assume universal vaccination
\item \textbf{Ceiling at 0.505}: hierarchical models combining mutation
and peptide levels do not exceed this ceiling---it is set by gate failures
(19.1\% of positives) and feature indistinguishability
\end{itemize}

\section{ML vs Simple Rule: Stability Analysis}

Across 50 random seeds, GBT achieves mean recall@20 of 0.531 $\pm$ 0.016,
while alt\_support $\times$ TPM achieves a deterministic 0.550. The simple
rule beats ML in 84\% of seeds. The previously reported ML ``SOTA'' of
0.578 was a top-2\% seed. At $N=213$ positives, ML overfits to the
negative sampling and introduces more variance than signal.

\textbf{Recommendation}: do not use ML for neoantigen prioritization
at this sample size. Use alt\_support $\times$ TPM with TCGA rescue
for mutations lacking RNA-seq variant evidence (recall@20 = 0.574).
...
\item Call variants with patient-matched HLA alleles
\item \textbf{Rank mutations}: by alt\_support $\times$ TPM.
For mutations without alt support, use TCGA expression $\times$ CSCAPE
\item \textbf{Select peptides}: for each top mutation, pick the
strongest MHC-binding peptide
\item \textbf{Synthesize}: top 20 peptides for vaccine
\item Expected yield: $\sim$57\% of immunogenic mutations in 20 candidates
\end{enumerate}
...
At mutation level, ranking by alt\_support $\times$ TPM---the product
of variant allele RNA-seq reads and gene expression---achieves 0.550
recall@20 with \textit{zero machine learning}. This matches the GBT
model (0.540) and approaches the exhaustive-search optimum (0.578).
ML adds only +0.028 recall beyond this trivial formula.

This reveals the central biological principle: neoantigen immunogenicity
is overwhelmingly determined by \textbf{total mutant mRNA production}.
Binding, stability, foreignness, driver status, and clonality are either
redundant with expression or noise. The clinical implication is that
RNA-seq alone (\$50) may suffice for neoantigen prioritization---MHC
binding prediction is valuable only for selecting which peptide to
synthesize from each prioritized mutation.
...
prioritization at peptide level, and 0.55 at mutation level---an 83\%
improvement over the binding-only standard of care. However, the
simplest competitive approach is merely ranking mutations by
alt\_support $\times$ TPM (0.550 recall@20, zero ML). The gate alone
provides 13.6$\times$ enrichment while eliminating 98.2\% of candidates.
These results are validated across three independent datasets with
consistent cross-dataset transfer. The deepest finding: neoantigen
immunogenicity is primarily determined by mutant mRNA abundance.
