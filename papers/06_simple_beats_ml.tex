...
We present evidence from 122 commits of systematic experimentation that,
at current sample sizes ($N=213$ immunogenic mutations), a formula requiring
zero training---simply multiplying variant allele RNA-seq support by gene
expression---matches or exceeds every ML approach we tested.
...
prioritization. The formula---variant allele support $\times$ gene
expression, with TCGA rescue and a gene blacklist---achieves 0.578
recall@20, is deterministic, requires no training, and runs in
milliseconds. ML may surpass simple rules when datasets grow beyond
$\sim$1,000 positives; until then, the field should focus on
generating more validated immunogenicity data rather than developing
