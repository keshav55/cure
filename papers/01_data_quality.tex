...
\bottomrule
\end{tabular}
\caption{Impact of HLA allele selection (MHCflurry presentation score, same scorer). Allele matching ($\Delta$AUROC = 0.040 from 12$\to$1) contributes more than allele count ($\Delta$AUROC = 0.032 from 3$\to$12). Patient allele included in 12-allele set 37\% of the time.}
\end{table}

With three default alleles (A*02:01, A*03:01, A*11:01), only 19\% of peptides were evaluated against their correct restricting allele. Expanding to 12 common alleles increases overlap to 37\%. Using the patient's actual allele achieves 100\% overlap and the highest AUROC.

Note: an earlier analysis comparing our full multi-feature scorer at 3 alleles (AUROC = 0.156) against NeoRanking binding rank at patient alleles (0.968) conflated allele mismatch with scorer complexity. The controlled comparison above isolates the allele matching effect using the same scorer (MHCflurry).
...
\bottomrule
\end{tabular}
\caption{Precision-recall at clinical operating points (our scorer, patient-matched alleles, downsampled evaluation: 92 pos, 810 neg). AUPRC = 0.691 on downsampled data (positive rate 10.2\%). Full dataset positive rate is 0.076\%.}
\end{table}

At the clinical operating point (threshold = 0.59), the scorer achieves 82\% recall with 49\% precision: of 20 predicted immunogenic peptides, approximately 10 would trigger T-cell responses. Note: precision-recall metrics are evaluated on downsampled data (10.2\% positive rate vs.\ 0.076\% in the full dataset) and would shift at the true class ratio.
