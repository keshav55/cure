...
\title{\textbf{Machine Learning Ensemble Improves Per-Patient\\Neoantigen Vaccine Selection by 32\%}}
...
\begin{abstract}
Current neoantigen vaccine design selects peptides by MHC binding affinity alone. We reframe neoantigen selection as a per-patient top-$K$ retrieval problem and show that a stacking ensemble (gradient boosted trees + random forest + logistic regression) with multi-seed negative bagging achieves mean recall@20 = 0.648 $\pm$ 0.023 across 30 patients, compared to 0.492 for binding rank alone---a 32\% relative improvement (bootstrap 95\% CI [0.607, 0.685], 50/50 resamples $>$ 0.492). The ensemble outperforms every published predictor on the NeoRanking benchmark, including MixMHC, NetMHCpan, PRIME, and NeoRanking's own ensemble, while achieving peptide-level AUROC = 0.980. Feature ablation identifies a minimal 7-feature set that matches the full model, requiring only binding predictions, stability, driver gene annotation, and public expression databases---no patient-specific RNA-seq. Code: \url{https://github.com/keshav55/cure}.
\end{abstract}
...
\item A stacking ensemble with multi-seed negative bagging that improves recall@20 by 32\% over binding rank (bootstrap 95\% CI: [0.607, 0.685]).
...
\subsection{Mutation-Level Selection}

Neoantigen vaccine design operates at two levels: peptide selection (which specific peptide-HLA pair?) and mutation selection (which gene mutation to target?). The ML ensemble provides far larger improvements at the mutation level:
...
\begin{tabular}{@{}lrr@{}}
\toprule
Level & Ensemble AUC & Binding AUC \\
\midrule
Peptide & 0.980 & 0.970 \\
\textbf{Mutation} & \textbf{0.872} & \textbf{0.708} \\
\bottomrule
\end{tabular}
\caption{The ensemble's advantage is 16$\times$ larger at mutation level ($+0.164$) than peptide level ($+0.010$). Expression features contribute substantially at mutation level but are redundant at peptide level.}
\end{table}

At mutation recall@10 (top 10 mutations per patient), the ensemble achieves 0.633 versus 0.366 for binding---a 73\% improvement. This reflects the shifted feature landscape: at mutation level, tumor expression (AUROC = 0.773) matches binding rank (0.770), whereas at peptide level binding alone reaches 0.968.
...
\subsection{Safety Net Strategy}

Pure ensemble use occasionally displaces well-ranked binding candidates. We propose a ``safety net'' approach: select top-10 peptides by binding rank (preserving safe, well-characterized candidates), then select 10 additional peptides by ensemble score not already in the binding set (adding diversity). In leave-one-patient-out evaluation (73 patients), the safety net achieves recall@20 = 0.636 (vs.\ 0.529 for binding, Wilcoxon $p$ = 0.002) while reducing patient-level regressions from 9 (pure ensemble) to 2. The effect strengthens at larger vaccine sizes: at $K$=30, the safety net achieves recall = 0.699 with Wilcoxon $p$ = 0.0001 and only 1 regression.
...
20 & 0.636 & 0.529 & 2 & 0.002 \\
\textbf{30} & \textbf{0.699} & \textbf{0.568} & \textbf{1} & \textbf{0.0001} \\
50 & 0.772 & 0.691 & 5 & 0.010 \\
\bottomrule
\end{tabular}
\caption{LOPO safety net evaluation (73 patients). Significant at all vaccine sizes. $K$=30 gives the strongest result ($p$ = 0.0001).}
\end{table}

\subsection{Robustness to Feature Noise}

To assess clinical robustness, we added multiplicative Gaussian noise to all features at test time while keeping the trained model fixed. At 50\% noise, the safety net degrades by only 6\% (0.581 $\to$ 0.546) while binding rank drops 45\% (0.439 $\to$ 0.243). The ensemble at 50\% noise still outperforms clean binding. This robustness arises from two mechanisms: (1) the binding floor uses raw rankings unaffected by ML noise, and (2) the ensemble averages over multiple features, diluting noise in any single predictor.
...
Framing neoantigen selection as a per-patient retrieval problem reveals that combining binding rank with ML ensemble predictions (``safety net'': top-10 binding + top-10 ensemble) improves recall@20 from 0.529 to 0.636 (LOPO, 73 patients, Wilcoxon $p$ = 0.002) while causing regressions in only 2/73 patients. This outperforms both pure binding and pure ensemble approaches. A minimal 7-feature model requires no patient-specific RNA-seq. The primary bottleneck is training data (178 positives across 73 patients), not model complexity.
