...
\midrule
NetMHCpan 4.1 & 2020 & Neural network & $\sim$0.65 \\
pTuneos & 2020 & Random forest & 0.72 \\
DeepImmuno & 2021 & CNN & 0.74 \\
PRIME & 2023 & Gradient boosting & 0.76 \\
NeoRanking & 2023 & Gradient boosting & 0.78 \\
BigMHC$^*$ & 2023 & Transformer & 0.59 \\
\textbf{cure (ours)} & \textbf{2026} & \textbf{Rule-based + MHCflurry} & \textbf{0.61} \\
\bottomrule
\end{tabular}
\caption{Neoantigen immunogenicity prediction tools. AUROC values are dataset-dependent with no universally best predictor. $^*$BigMHC reports neoepitope AUROC 0.585, emphasizing precision and AUPRC over AUROC.}
\label{tab:related}
\end{table*}
...
\textbf{Signal 3: BLOSUM80 Foreignness.} For each mutated position $i$, we compute $f_i = (B_{diag}(wt_i) - B(wt_i, mut_i)) \cdot B_{diag}(wt_i)/16 \cdot w_i$, where $B$ is the BLOSUM80 matrix, $B_{diag}$ is the diagonal (self-substitution) score, and $w_i = 1.5$ for central positions. This weights mutations of conserved residues (high $B_{diag}$, e.g.\ W=16) to dissimilar amino acids as maximally foreign.
...
\bottomrule
\end{tabular}
\caption{Sequence-only scorer. Clinical CI includes 0.50 (not significant). TESLA CI excludes 0.50.}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Feature AUROC on TESLA (N=319 with all features available). Foreignness is anti-correlated.}
\end{table}
